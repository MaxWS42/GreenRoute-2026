...
def green_score(distance_m, duration_s, profile, fastest_duration_s):
    emissions_map = {
        "driving-car": 0.192,
        "cycling-regular": 0.0,
        "foot-walking": 0.0
    ...
    practicality_weight = min(0.1, max(0.1, distance_km / 10))
    eco_weight = 1 - practicality_weight

    emissions = distance_m * (emissions_map.get(profile, 0.15) / 1000)
    eco_rating = 100 * (2.718 ** (-emissions/10))
    practicality = (fastest_duration_s / duration_s)**10 * 100

    return (eco_weight * eco_rating) + (practicality_weight * practicality)
...
def route():
    ...
    fastest = min(results, key=lambda x: x["duration"])["duration"]

    for r in results:
        r["score"] = green_score(r["distance"], r["duration"], r["profile"], fastest)
...
def select_route():
    ...
    user.green_score += score
    user.journey_count += 1
    db.session.commit()

    return jsonify({
        "user_score": user.green_score / user.journey_count
    })
...
def leaderboard():
    users = User.query.filter(User.journey_count > 0).order_by(User.green_score.desc()).limit(10).all()

    return jsonify([
        {"username": u.username, "score": u.green_score / u.journey_count}
        for u in users
    ])
